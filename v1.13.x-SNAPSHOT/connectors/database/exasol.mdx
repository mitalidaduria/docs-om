...
- [Troubleshooting](/v1.13.x-SNAPSHOT/connectors/database/exasol/troubleshooting)
<ExternalIngestionDeployment />
<Card title="OpenMetadata 1.6.1 or later" href="/v1.13.x-SNAPSHOT/deployment" >
...
OpenMetadata fetches the query logs for the lineage workflow by querying `EXA_DBA_AUDIT_SQL`
and joining `EXA_DBA_AUDIT_SESSIONS`. To access these system tables, an Exasol user
must have the `SELECT ANY DICTIONARY` system privilege, which is typically reserved
for database administrators, and auditing must be enabled for the database. For further details, refer to the
[Auditing page](https://docs.exasol.com/db/latest/database_concepts/auditing.htm#Implementation).
## Metadata Ingestion
